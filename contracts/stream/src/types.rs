...
pub struct Stream {
    ...
    // NOTE: the on-complete callback (`on_complete_contract` / `on_complete_function`)
    // lives in separate persistent storage (see `storage::get_on_complete` /
    // `storage::set_on_complete`), not as Stream fields — same treatment as
    // `stream_tag` above — to keep this struct under Soroban's 40-field UDT limit.
}
